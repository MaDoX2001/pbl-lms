# Create Project 9 - Team project with dual-phase evaluation
...
$projectJson = Get-Content -Path "$scriptDir\project-9-data.json" -Encoding UTF8 -Raw
$groupCardJson = Get-Content -Path "$scriptDir\project-9-group-card.json" -Encoding UTF8 -Raw
$individualCardJson = Get-Content -Path "$scriptDir\project-9-individual-card.json" -Encoding UTF8 -Raw

Write-Host "`n=== Creating Project 9 ===" -ForegroundColor Cyan
...
Write-Host "`nProject 9 setup complete" -ForegroundColor Green
