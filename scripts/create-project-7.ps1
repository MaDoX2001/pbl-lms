# Create Project 7 - Team project with dual-phase evaluation
...
$projectJson = Get-Content -Path "$scriptDir\project-7-data.json" -Encoding UTF8 -Raw
$groupCardJson = Get-Content -Path "$scriptDir\project-7-group-card.json" -Encoding UTF8 -Raw
$individualCardJson = Get-Content -Path "$scriptDir\project-7-individual-card.json" -Encoding UTF8 -Raw

Write-Host "`n=== Creating Project 7 ===" -ForegroundColor Cyan
...
Write-Host "`nProject 7 setup complete" -ForegroundColor Green
