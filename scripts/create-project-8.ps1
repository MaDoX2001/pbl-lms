# Create Project 8 - Team project with dual-phase evaluation
...
$projectJson = Get-Content -Path "$scriptDir\project-8-data.json" -Encoding UTF8 -Raw
$groupCardJson = Get-Content -Path "$scriptDir\project-8-group-card.json" -Encoding UTF8 -Raw
$individualCardJson = Get-Content -Path "$scriptDir\project-8-individual-card.json" -Encoding UTF8 -Raw

Write-Host "`n=== Creating Project 8 ===" -ForegroundColor Cyan
...
Write-Host "`nProject 8 setup complete" -ForegroundColor Green
