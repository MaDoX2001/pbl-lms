# Create Project 10 - Team project with dual-phase evaluation
...
$projectJson = Get-Content -Path "$scriptDir\project-10-data.json" -Encoding UTF8 -Raw
$groupCardJson = Get-Content -Path "$scriptDir\project-10-group-card.json" -Encoding UTF8 -Raw
$individualCardJson = Get-Content -Path "$scriptDir\project-10-individual-card.json" -Encoding UTF8 -Raw

Write-Host "`n=== Creating Project 10 ===" -ForegroundColor Cyan
...
Write-Host "`nProject 10 setup complete" -ForegroundColor Green
